...
import { initializeFirebase, createThreadForCoin, getComments, addComment, getThread, uploadImage, getAllThreads, getDb, addDevLog, getDevLogs, deleteDevLog, pinThread } from './firebase.js';
...
app.post('/api/launch/prepare', launchLimiter, async (req, res) => {
  try {
    const { wallet, name, symbol, description, image, creatorUsername, twitter, devBuy, board } = req.body;
    const VALID_BOARDS = ['meme', 'tek', 'animal', 'trump', 'elon', 'random', 'sol', 'ai', 'pol', 'degen', 'moon'];
    const boardId = VALID_BOARDS.includes(board) ? board : 'random';
...
app.get('/api/threads', async (req, res) => {
  try {
    ...
    if (board) enrichedThreads = enrichedThreads.filter(t => t.board === board);
...
// Pin/unpin a thread
app.post('/api/admin/pin-thread', async (req, res) => {
  try {
    const secret = req.headers['x-admin-secret'];
    if (!ADMIN_SECRET || secret !== ADMIN_SECRET) {
      return res.status(401).json({ success: false, error: 'Unauthorized' });
    ...
    const { mint, pinned } = req.body;
    if (!mint) {
      return res.status(400).json({ success: false, error: 'Missing mint address' });
    }

    await pinThread(mint, pinned !== false);
    console.log(`[Admin] Thread ${mint} ${pinned !== false ? 'pinned' : 'unpinned'}`);
    res.json({ success: true });
  } catch (error) {
    console.error('Admin pin-thread error:', error);
    res.status(500).json({ success: false, error: error.message });
  }
});
