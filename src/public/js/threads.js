...
function renderThreads() {
  ...
  container.innerHTML = '<div class="threads-grid">' + list.map((thread, idx) => {
      ...
      <div class="thread" id="thread_${idx}">
        <div class="post op">
          ...
          <blockquote class="postMessage" style="${bodyMargin}">
            ${excerpt ? escapeHtml(excerpt) : '<span class="quote">&gt; no description provided</span>'}
          </blockquote>
          <span class="omitted">
            <span id="vol-${thread.mint}"></span>
            ${replies > 0
              ? `${replies} ${replies === 1 ? 'reply' : 'replies'} omitted. <a href="/thread/${thread.mint}">Click here to view.</a>`
...
async function updateCoinStats(mint) {
  try {
    ...
    if (volEl && coin.volume24h) volEl.textContent = `Vol: $${formatNum(coin.volume24h)} | `;
  } catch (e) { /* ignore */ }
}
